...
def upgrade():
    ...
    # document_template (with its index) from the current models, which match
    # this schema exactly — so only create when it's missing.
    if 'document_template' not in existing:
    ...
    #
    # Only attempt the copy when invoice_template still exists (on a schema
    # where create_all built the current models, it never existed), and only
    # insert 'invoice' when it isn't already present (avoids a duplicate on
    # the unique kind index if create_all/seeding got there first).
    if 'invoice_template' in existing:
        already = bind.execute(sa.text(
            "SELECT 1 FROM document_template WHERE kind = 'invoice' LIMIT 1"
        )).fetchone()
